...
export default function MessageBubble({ message, currentUserEmail }) {
  ...
  return (
    <div className={cn("flex gap-3 mb-4", isOwnMessage ? "justify-end" : "justify-start")}>
      {!isOwnMessage && (
        <div className="h-8 w-8 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">
                ...
                code: ({ inline, className, children, ...props }) => {
                  ...
                  return !inline && match ? (
                    ...
                    <code className={cn(
                      ...
                      isOwnMessage ? "bg-blue-400 bg-opacity-30" : "bg-slate-100 text-slate-700"
                    )}>
                      {children}
                    </code>
                  );
            ...
            {message.file_urls.map((url, idx) => (
              <div key={idx} className="rounded-lg overflow-hidden border border-slate-200">
                {url.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                  <img src={url} alt="Attachment" className="max-w-xs max-h-64 object-cover" />
                ) : (
                  <a href={url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-3 hover:bg-slate-50">
                    <span className="text-sm text-blue-600">📎 Attachment {idx + 1}</span>
                  </a>
                )}
              </div>
            ))}
      ...
      {isOwnMessage && (
        <div className="h-8 w-8 rounded-full bg-gradient-to-br from-indigo-400 to-indigo-600 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">
          {message.sender_email[0].toUpperCase()}
        </div>
      )}
    </div>
